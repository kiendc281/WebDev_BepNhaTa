...
export class LenThucDonComponent implements OnInit {
  ...
  updateVisibleMenus(): void {
    ...
    this.visibleMenuDays = this.allMenuDays.slice(
      startIndex,
      startIndex + this.menusPerPage
    );
  }
  ...
  buyMenu(menuDay: MenuDay): void {
    ...
    if (!menuDay.meals || menuDay.meals.length === 0) {
      this.showNotification(
        'Không có sản phẩm nào trong thực đơn này',
        'error'
      );
      return;
    }
    ...
    this.showNotification(
      'Đang xử lý thêm sản phẩm vào giỏ hàng...',
      'success'
    );

    // Tạo bản sao để không ảnh hưởng đến dữ liệu gốc
    const mealItems = [...menuDay.meals].filter(
      (meal) => meal.product && meal.product._id
    );

    // Nếu không có sản phẩm hợp lệ
    if (mealItems.length === 0) {
      this.showNotification(
        'Không có sản phẩm hợp lệ nào trong thực đơn này',
        'error'
      );
      return;
    }
    ...
    console.log(
      `Chuẩn bị xử lý ${mealItems.length} sản phẩm từ thực đơn ngày ${menuDay.day}`
    );
    ...
    const finalizeProcess = () => {
      ...
      allProcessed = true;
      console.log(
        `Đã hoàn thành việc xử lý: ${productsAdded} thành công, ${failedProducts} thất bại`
      );
      ...
      if (productsAdded > 0) {
        ...
        if (outOfStockProducts.length > 0) {
          ...
          message += `. Các sản phẩm: ${outOfStockProducts.join(
            ', '
        ...
        if (outOfStockProducts.length > 0) {
          this.showNotification(
            `Không thể thêm sản phẩm vào giỏ hàng. Các sản phẩm: ${outOfStockProducts.join(
              ', '
            )} không đủ số lượng trong kho.`,
            'error'
          );
          ...
          this.showNotification(
            'Không thể thêm sản phẩm vào giỏ hàng. Vui lòng kiểm tra lại hàng tồn kho.',
            'error'
          );
        }
      } else {
    ...
    const processNextItem = (index: number) => {
      ...
      if (this.authService.isLoggedIn()) {
        ...
        this.userCartService
          .addToCart(product, quantity, servingSize, finalPrice)
          .subscribe({
            next: () => {
            ...
            error: (error) => {
              ...
              console.error(
                `Lỗi khi thêm sản phẩm ${this.getProductTitle(product)}:`,
                error
              );
        ...
        try {
          this.guestCartService.addToLocalCart(
            product,
            quantity,
            servingSize,
            finalPrice
          ...
          console.log(
            `Thêm thành công vào giỏ khách: ${this.getProductTitle(product)}`
          );
          productsAdded++;
        } catch (error) {
          failedProducts++;
          ...
          // Chi tiết lỗi để debug
          console.error(
            `Lỗi khi thêm sản phẩm ${this.getProductTitle(
              product
            )} vào giỏ khách:`,
            error
          );
  ...
  calculateProductPrice(product: Product, servingSize: string): number {
    ...
    // 2. Nếu không có trong pricePerPortion, thử lấy từ pricePerPortionArray
      ...
      product.pricePerPortionArray &&
      Array.isArray(product.pricePerPortionArray) &&
      product.pricePerPortionArray.length > 0
    ) {
      // Tìm khẩu phần phù hợp
      const portionItem = product.pricePerPortionArray.find(
        (p) => p.portion === servingSize
      );
      if (portionItem) {
    ...
    this.suggestedRecipes.forEach((recipe) => {
      if (recipe && recipe._id) {
        this.favoritesService
          .checkFavorite(recipe._id, 'recipe')
          .subscribe((isSaved) => {
            if (isSaved) {
  ...
  addToCart(product: Product): void {
    ...
    if (this.authService.isLoggedIn()) {
      ...
      this.userCartService
        ...
        .subscribe({
          next: (cart) => {
            ...
            const productName =
              product.ingredientName || product.title || 'Sản phẩm';
            this.showNotification(
          ...
          error: (error) => {
            console.error(
              'Lỗi khi thêm sản phẩm vào giỏ hàng người dùng:',
              error
            );
            ...
            if (error && typeof error === 'object') {
              ...
              if (
                errorMessage.includes('không đủ số lượng') ||
            ...
            this.showNotification(
              `Không thể thêm sản phẩm vào giỏ hàng: ${errorMessage}`,
              'error'
            );
          },
        });
      ...
      try {
        ...
        console.log(
          'Đã thêm sản phẩm vào giỏ hàng khách:',
          product.ingredientName || product.title
        );
        const productName =
          product.ingredientName || product.title || 'Sản phẩm';
