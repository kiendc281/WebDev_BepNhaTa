...
export class FavoritesComponent implements OnInit {
  ...
  loadFavorites(type: string): void {
    ...
    this.favoritesService.getFavoritesWithDetails(type).subscribe({
      next: (data) => {
        console.log(`Đã nhận ${data.length} mục yêu thích ${type} với chi tiết:`, data);
  ...
  enrichMissingDetails(favorites: FavoriteItem[], type: string): void {
    ...
    console.log(`Cần bổ sung chi tiết cho ${missingDetailsItems.length} mục ${type}`);
    ...
    forkJoin(requests).pipe(
      ...
      results.forEach((result, index) => {
        ...
        const favoriteIndex = this.favorites.findIndex(f => f.targetId === targetId);
        ...
        if (favoriteIndex >= 0) {
          ...
          if (type === 'recipe') {
            ...
            const processedResult = {
              ...blogResult,
              author: typeof blogResult.author === 'object' ? 'Bếp Nhà Ta' : (blogResult.author || 'Bếp Nhà Ta')
            };
            this.favorites[favoriteIndex].details = processedResult;
          } else {
            this.favorites[favoriteIndex].details = result;
          }
          console.log(`Đã cập nhật chi tiết cho ${type} ID ${targetId}:`, this.favorites[favoriteIndex].details);
        }
      });
    });
  }
  ...
  removeFromFavorites(favorite: FavoriteItem): void {
    if (confirm('Bạn có chắc chắn muốn xóa mục này khỏi danh sách yêu thích?')) {
      this.favoritesService.removeFromFavorites(favorite.targetId, favorite.type).subscribe({
        next: (response) => {
          ...
          if (response.success) {
            ...
            alert(response.message || 'Không thể xóa khỏi danh sách yêu thích.');
          }
        },
        error: (err) => {
          console.error('Lỗi khi xóa khỏi yêu thích:', err);
          alert('Đã xảy ra lỗi khi xóa khỏi danh sách yêu thích. Vui lòng thử lại sau.');
        }
      });
    }
  }
